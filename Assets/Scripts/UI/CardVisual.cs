...
namespace DualCraft.UI
{
    ...
    public class CardVisual : MonoBehaviour
    {
        ...
        private static readonly Dictionary<string, Material> _holoCache = new();

        private static Material LoadHoloMaterial(Rarity rarity)
        {
            string matName = rarity switch
            {
                Rarity.Rare      => "CardHolo_Rare",
                Rarity.Epic      => "CardHolo_Epic",
                Rarity.Legendary => "CardHolo_Legendary",
                _                => null,
            };
            if (matName == null) return null;
            if (_holoCache.TryGetValue(matName, out var cached)) return cached;
            var mat = Resources.Load<Material>("Materials/" + matName);
            if (mat == null) mat = Resources.Load<Material>(matName);
            if (mat != null) _holoCache[matName] = mat;
            return mat;
        }
        ...
        private void Refresh()
        {
            ...
            if (artworkImage)
            {
                ...
                // Apply holographic material for rare+ cards
                var holoMat = LoadHoloMaterial(_cardData.rarity);
                if (holoMat != null)
                    artworkImage.material = holoMat;
                else
